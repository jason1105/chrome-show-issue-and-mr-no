...
- B1 a11y 增量 = `8283302`：恰 2 文件（`src/badge-css.js` +5/−4、`src/options.css` +3/−1，合计 +8/−5）
...
- C 线主题跟随真实实现在 **`src/content.js:154` `resolveGitLabTheme()`**（读 `<html data-theme>`、兜底 `gl-dark` class）**+ `content.js:2029-2030` `themeObserver`** 运行时跟随，并落 host `data-theme` 属性（content.js:171）。`badge-css.js` 本波**无** `:host([data-theme])` 深色规则；`badge-css.js:593` 的 `@media (prefers-color-scheme: dark)` 是现状已有 OS 级适配，非 C 线产物。`:host([data-theme])` 深色规则属 B2 后续。
...
- **5 张截图**（`screenshots/issue11-release/`，1280×800）：

| 图 | 文件 | 内容 | 主题 |
|---|---|---|---|
| 01 主面板 | `issue11-01-main-panel.png` | 展开面板 Open-only 默认视图，当前项 #8 `Current` 浅蓝高亮 | 浅色 |
| 02 展开面板 | `issue11-02-panel-expanded.png` | 面板「All states」状态筛选视图（**注**：本图面板内 5 行数据仍为全部 Open；"All states" 为筛选器标题，实际数据未含 Closed/Merged，仅作筛选控件演示，不代表全状态数据） | 浅色 |
...
| 05 拼图 | `preview-grid.png` | 上述 4 图拼合预览网格 | 浅色 |
...
- **真机「浅色主题」截图已补齐**（5 张，见第三节），作为验收标准①的可视化证据，供 @admin 查看样式。
