...
Issue/MR 列表快速跳转。还需确认滚动、输入方式、接口失败、缓存刷新和 GitLab 局部
导航过程中的状态正确，同时确认 Manifest V3、脚本加载顺序、权限和图标均可由
Chromium 正确读取。
...
2026-07-31 实际结果：36 项通过，0 项失败，0 项跳过。
...
9. 悬停复制按钮时 tooltip 在按钮下方显示“复制 #123”；点击后剪贴板为 `#123`，
   图标变为绿色对勾。
10. 滚动到 `1800px` 后，标签顶部坐标与滚动前一致。
11. 键盘可以打开列表，Escape 关闭后焦点回到编号按钮。
12. SPA 风格切换到 MR 子页面后，唯一标签更新为 `MR !456`，缓存不重复请求，当前
    标记移到 MR，复制反馈和剪贴板内容同步更新。
13. 真实点击其他 MR 链接后在当前标签页跳转，固定编号同步更新。
14. 离开详情页进入 Issue 列表后，标签被移除。
15. 测试结束后扩展被卸载，WebDriver 会话、ChromeDriver、HTTP 服务与临时目录
    均被清理。
...
7. 悬停和键盘聚焦复制按钮，确认 tooltip 位于按钮下方且文案为“复制 #编号”。
8. 点击 Issue 复制按钮，确认可粘贴 `#编号`，图标变为绿色对勾并自动恢复。
9. 打开 `gitlab.com` 的一个登录态 MR，确认显示 `MR !编号`，列表当前项随之更新，
   并可复制 `!编号`。
10. 在实际 HTTP 自建 GitLab（例如 `http://git.tsintergy.com`）重复导航和复制，确认
    GitLab API 使用现有登录态，Clipboard API 不可用时降级复制仍可工作。
11. 模拟离线或 API 失败，确认列表显示错误，但顶部编号和复制按钮仍可使用。
12. 从详情页使用 GitLab 页面内链接切换到另一个 Issue/MR，确认编号、列表当前项、
    复制文案和反馈状态同步更新。
13. 打开 Issue/MR 列表页、新建页、编辑页和普通网站，确认标签不显示。
14. 在浅色和深色系统主题下确认标签、列表、当前项和绿色对勾均清晰可辨。
