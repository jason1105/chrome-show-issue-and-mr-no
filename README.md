...
> 🌍 **中文** · [English](#english-english) ｜ 📦 零构建、零追踪、零数据上传的 Manifest V3 扩展
...
<h2 id="english-english" align="center">🇬🇧 English</h2>

<details>
<summary><b>Click to expand the English version</b>（点击展开英文版）</summary>
...
A zero-build, zero-tracking, zero-upload **Manifest V3** browser extension. While switching between GitLab Issues and Merge Requests, it pins the current number at the **top of the page** and keeps it visible even when you scroll.
...
- **Floating number badge** — always visible while scrolling; auto-detects modern & legacy GitLab URLs; supports multi-level namespaces and self-hosted domains
- **One-click copy** — copy `#123` / `!456` with visual feedback (✓ copied), graceful clipboard fallback on HTTP contexts
- **Project-wide navigation** — hover to list all Open Issues & MRs; search by title or `#123` / `!456`; filter All/Issue/MR locally (no extra API calls); refresh bypasses cache
- **Draggable, position-aware** — drag the handle to snap to top/left/right, position persists across pages & restarts; double-click resets to top-center
- **Full-repo mode** — enable to show the number hint on all repository pages
- **Localized & configurable** — multilingual UI, rich options
- **Zero build, zero dependency, zero tracking** — minimal permissions (`storage` + `scripting`), all host permissions optional
...
#### Manual load (developers)

**Chrome** — `chrome://extensions` → enable Developer mode → "Load unpacked" → select this repo root (contains `manifest.json`) → refresh GitLab pages.

**Arc** — `arc://extensions` → Developer mode → "Load unpacked" → select repo root → refresh.

### ⚙️ Configuration

Open the options page from the extension detail to adjust default filter, search scope, cache TTL, load mode, etc. Settings are stored in `chrome.storage.sync` and synced with your Chrome account — never overridden by remote config.

### 🔐 Privacy

Content script matches all HTTP/HTTPS sites so self-hosted GitLab works out of the box. It parses the page URL, reads only the current project's Open Issues/MRs via the same-origin GitLab REST API v4, reuses your existing GitLab session (no token/Cookie stored), caches IID/title/URL in memory only, and **never uploads any data**. No `tabs`/`cookies`/`identity` permissions. No remote code, analytics, or runtime dependencies.
...
See [Testing](docs/testing.md) for the full matrix.

### 📄 License

Released under the **MIT License**. See [LICENSE](LICENSE) — `Copyright (c) 2026 jason1105`.
